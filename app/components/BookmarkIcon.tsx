// The bookmark glyph from Figma (public/bookmark.svg), inlined so its fill can
// be toggled in React: outline when unsaved, filled olive when saved. Purely
// presentational — the parent owns the saved state and the click handler.
export default function BookmarkIcon({
  filled = false,
  className,
}: {
  filled?: boolean;
  className?: string;
}) {
  return (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill={filled ? "#41482A" : "none"}
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      aria-hidden
    >
      <path
        d="M14.1667 2.5C14.6088 2.5 15.0327 2.67559 15.3453 2.98816C15.6578 3.30072 15.8334 3.72464 15.8334 4.16667V16.6667C15.8334 16.8126 15.795 16.956 15.7221 17.0824C15.6493 17.2089 15.5445 17.314 15.4183 17.3872C15.2921 17.4604 15.1488 17.4992 15.0029 17.4997C14.857 17.5002 14.7135 17.4624 14.5867 17.39L10.8267 15.2417C10.575 15.0978 10.29 15.0222 10.0001 15.0222C9.71013 15.0222 9.42519 15.0978 9.17342 15.2417L5.41341 17.39C5.2867 17.4624 5.1432 17.5002 4.99727 17.4997C4.85133 17.4992 4.70809 17.4604 4.58187 17.3872C4.45564 17.314 4.35086 17.2089 4.27801 17.0824C4.20516 16.956 4.1668 16.8126 4.16675 16.6667V4.16667C4.16675 3.72464 4.34234 3.30072 4.6549 2.98816C4.96746 2.67559 5.39139 2.5 5.83341 2.5H14.1667Z"
        stroke="#41482A"
        strokeWidth="1.6"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}
